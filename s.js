...
$(document).ready(function(){
  ...
  // pair
    ...
    const mainpair = (async function(bh){
      ...
      await $.post("s.php", {bh: bh}, re=>{switch(re){
        case "无法连接至服务器。": case "服务器出错，请尝试换一个房间号。": case "服务器拥挤，请稍后再试。":
          errwin(re,divid);x=undefined; return;
        default:
          if(re.slice(0,1)=="m"){
            ...
            $.post("s.php", {newroom: re, newbh: bh}, nr=>{switch(nr){
              case "无法连接至服务器。": case "服务器：房间创建失败。": errwin(re,divid);let x=undefined;return;
              default:
                ...
                itv=setInterval(async ()=>{
                  await $.post("s.php", {waitroom: match}, conb => {
                    switch (conb) {
                      ...
                      case "服务器出现错误。": errwin(conb,divid);return;
    ...
    $("#errbtnout").click(()=>{
      leaving();
      window.location.replace("https://thisoe.dev/project/");
    });
    $("#errbtnredo").click(()=>{
      $("#errdiv").hide();
  ...
  const runmain = (()=>{
    ...
    $(document).on("click","#bye",()=>{
      if (confirm("将永久删除此房间的唠嗑记录。\n确定退出？")) {leaving();window.location.replace("https://thisoe.dev/project/");}
    });
