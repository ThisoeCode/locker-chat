...
if( !(
  ...
  isset($_POST["tos"]) ||
  isset($_POST["cancel"]) ))
  {echo 0;header("location: https://thisoe.dev/project/");die;}else $_GET['acc']=0;
...
elseif(isset($_POST["newroom"])){
  ...
  $stmt->bind_param('sii', $_POST["newbh"],$f,$dt);
  if($stmt->execute()){
    echo $f;
  }else{
    echo "服务器：房间创建失败。";
...
elseif(isset($_POST["cancel"])){
  ...
  include_once "_hdl/insu.php";
    if ($in_su->query("SELECT * FROM lockerpair WHERE ".$where.$x)->num_rows > 0) {
      $in_su->query("DELETE FROM lockerpair WHERE ".$where.$x);
      echo $in_su->affected_rows;
    } else {echo 'ng';} // 没有该房
}
...
elseif(isset($_POST["gotin"])){
  $m=$_POST["gotin"];
  include_once "_hdl/insu.php";
  $in_su->query('UPDATE lockerpair SET roomno = "" WHERE ispaired=1 AND fileno='.$m);
  $a=$in_su->affected_rows;
// 造locker新表
  if ($a==1) {
    ...
    if ($in_su->query($q) === TRUE) {
      echo "1".$a;
    } else {echo "0".$in_su->error;} // 制表失败
  } else {echo "1".$a;}
}
